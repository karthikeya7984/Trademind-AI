...
Provider order (try each in sequence, skip on quota/auth error):
  ...
  2. OpenRouter    (mistralai/mistral-7b)       — fallback #1
  3. Mistral       (mistral-small-latest)       — fallback #2
  4. Grok / xAI   (grok-3-mini)                — fallback #3

Each provider receives the FULL context (raw market data + any partial
analysis accumulated from previous failed providers) so no data is lost
and the response quality stays consistent regardless of which provider
ultimately answers.

Previous pipelines (kept as comments):
# Stage 1 DeepSeek → Stage 2 Grok pipeline (commented out)
# Gemini pipeline (commented out)
# Claude/Anthropic pipeline (commented out)
"""
...
async def _waterfall(messages: list[dict]) -> str:
    """
    Try providers in order. Each failed provider's skip reason is noted.
    The full context (already embedded in messages) is passed to every provider
    so no data is lost between attempts.
    """
    providers = []
    ...
    for name, base, key, model, timeout, extra in providers:
        ...
        if result:
            return result
        # skip=True → quota/auth exhausted, try next
        # skip=False + None → transient error, still try next (graceful degradation)
...
def _hard_fallback(messages: list[dict]) -> str:
    ...
    if signal_line and price_line:
        return (
            ...
            f"{price_line}\n{signal_line}\n\n"
            f"*(Groq, OpenRouter, Mistral, and xAI are all unavailable. "
